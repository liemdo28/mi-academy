...
- Free Creativity now uses a dedicated participation-based story lab (`creative_story_lab`) with scene, character, feeling, and story-idea composition; it does not grade creativity as right/wrong, records progress as ungraded participation (`correct=null`, not true/false), saves in-progress composition through snapshots, and requires completed creative artifacts to persist in the local-first artifact store before completion is reported.
