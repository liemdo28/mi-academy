...
| Robot Commands game | ⚠️ | 10 validated levels, playable Flutter command-sequencing slice using `mi_blocks`; command editor now supports move/remove/reset/retry interactions, game-slice golden coverage, and privacy-safe offline snapshot save/restore coverage, but still needs device/integration proof |
...
| Static child-safety audit | ⚠️ | `python tools/child_safety_audit.py --json` passes across 32 files with 0 failures; 2 network/offline-review dependency warnings remain |
| Release evidence ledger | ⚠️ | `python tools/release_evidence.py --write --json` writes `docs/qa/RELEASE_EVIDENCE_2026-07-17.md` and correctly reports `blocked` until production audio and external/manual release evidence are complete |
...
| CI release gates | ⚠️ | `.github/workflows/ci.yml` now runs content validation, level solvability, content safety audit, game network audit, static child-safety audit, mobile platform privacy audit, child-safety pre-signoff, Python tests, mobile analyze/tests, web/APK builds, performance artifact baseline, and iOS no-codesign build; remote GitHub run proof still pending |
...
| Backend parent/sync data flows | ⚠️ | FastAPI app imports cleanly; local async API tests verify parent reports/weekly summaries, privacy-safe export, child deletion cascades, progress sync upsert, attempt sync idempotency, valid JSON answer persistence, and daily session sync upsert; deployed API proof still pending |
...
- Mobile platform privacy audit proving current main/release manifests and dependency files do not contain sensitive permissions, ad identifiers, tracking prompts, ad SDKs, IAP SDKs, or social SDKs
- Automated per-game child-safety pre-signoff report for the six MVP games, with manual release sign-off still pending
- Release evidence ledger that aggregates local gates and keeps production audio plus manual/device/deployed/remote CI proof visibly blocked
...
- GitHub Actions release-gate workflow configured for content, safety, platform privacy, tests, builds, and artifact baseline
...
- Backend sync tests for progress upsert, idempotent attempt ingestion, valid JSON answer persistence, and daily session upsert
- Mobile sync adapter tests for routing queued progress, attempt, and session-end items to the backend sync API while keeping snapshots local until a server contract exists
...
- Word Builder, Sound Match, Math Race, Math Supermarket, and Robot Commands session save/restore coverage proving JSON-round-tripped offline snapshots can resume progress without parent/email/token/raw-answer data
...
- Manual per-game child-safety checklist and sign-off after the automated pre-signoff report
...
- Deployed backend/device offline-sync verification
...
4. Add deployed backend/device offline-sync verification
