...
def validate_word_builder_content(content: dict, level_num: int) -> list[str]:
    ...
    for locale, loc_data in content.items():
        ...
        if not isinstance(letters, list) or not letters:
            errors.append(f"Level {level_num} ({locale}): letters must be a non-empty list")
...
def validate_sound_match_content(content: dict, level_num: int) -> list[str]:
    ...
    for locale, loc_data in content.items():
        ...
        if not isinstance(answer, str) or not answer:
            errors.append(f"Level {level_num} ({locale}): correctAnswer must not be empty")
        if not isinstance(options, list) or len(options) < 2:
            errors.append(f"Level {level_num} ({locale}): options must contain at least 2 items")
        else:
        ...
        if not isinstance(transcript, str) or not transcript:
            errors.append(f"Level {level_num} ({locale}): audioTranscript must not be empty")
    return errors
    ...
    for locale, loc_data in content.items():
        ...
        for option_index, option in enumerate(options):
            if not isinstance(option, dict):
                errors.append(f"Level {level_num} ({locale}) option {option_index}: must be a map")
                continue
        ...
        if len(option_texts) != len(set(option_texts)):
            errors.append(f"Level {level_num} ({locale}): option text values must be unique")
        if correct_count != 1:
            errors.append(
...
def validate_robot_commands_level(level: dict, level_num: int) -> list[str]:
    ...
    if not isinstance(grid, dict) or not isinstance(start, dict) or not isinstance(goal, dict):
    ...
    for field in ['obstacles', 'collectibles']:
        ...
        for index, point in enumerate(values):
            if not isinstance(point, dict):
                ...
                target.add((point.get('x'), point.get('y')))
    ...
    if isinstance(content, dict):
        for locale, loc_data in content.items():
            ...
            if not isinstance(commands, list) or not commands:
                errors.append(f"Level {level_num} ({locale}): availableCommands required")
            else:
            ...
            if not isinstance(required, list) or len(required) < 2:
                errors.append(f"Level {level_num} ({locale}): requiredCommands must contain START plus commands")
            else:
                if required[0] != 'START':
                    errors.append(f"Level {level_num} ({locale}): requiredCommands must start with START")
        ...
        if game_id == 'memory_cards':
            ...
            for locale, loc_data in content.items():
                if isinstance(loc_data, dict):
                    ...
                    if cards:
                        errors.extend(validate_card_pairs(cards, level.get('levelNumber', i + 1)))
                        break  # Only need to validate cards once per level
        elif game_id == 'word_builder':
            errors.extend(validate_word_builder_content(
                level.get('localizedContent', {}),
                level.get('levelNumber', i + 1),
            ))
        elif game_id == 'sound_match':
            errors.extend(validate_sound_match_content(
                level.get('localizedContent', {}),
                level.get('levelNumber', i + 1),
            ))
        elif game_id in {'math_race', 'math_supermarket'}:
            errors.extend(validate_choice_options(
                level.get('localizedContent', {}),
                level.get('levelNumber', i + 1),
            ))
        elif game_id == 'robot_commands':
            errors.extend(validate_robot_commands_level(
                level,
                level.get('levelNumber', i + 1),
            ))
...
def main():
    games = {
        'word_builder': PROJECT_ROOT / 'apps' / 'mobile' / 'assets' / 'levels' / 'word_builder.json',
        'sound_match': PROJECT_ROOT / 'apps' / 'mobile' / 'assets' / 'levels' / 'sound_match.json',
        'math_race': PROJECT_ROOT / 'apps' / 'mobile' / 'assets' / 'levels' / 'math_race.json',
        'math_supermarket': PROJECT_ROOT / 'apps' / 'mobile' / 'assets' / 'levels' / 'math_supermarket.json',
        'memory_cards': PROJECT_ROOT / 'apps' / 'mobile' / 'assets' / 'levels' / 'memory_cards.json',
        'robot_commands': PROJECT_ROOT / 'apps' / 'mobile' / 'assets' / 'levels' / 'robot_commands.json',
    }
