...
void main() {
  test(
    'Memory Cards restores an offline snapshot and can finish the level',
    () async {
      ...
      expect(
        restoredGame.cards.where((c) => c.state == CardState.matched),
        hasLength(2),
      );
  ...
  test(
    'Memory Cards snapshot keeps state privacy-safe for offline persistence',
    () async {
      final game = MemoryCardsGame();
      await _startMemoryGame(game);
