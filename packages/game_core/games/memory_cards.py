...
class MemoryCardsGame(GameEngineBase):
    ...
    def load_level(self, level):
        ...
        random.shuffle(deck)
        cards = [
            {"index": i, "symbol": s, "matched": False} for i, s in enumerate(deck)
        ]
        items = [
            {
                "cards": cards,
                ...
                "hint": "Nho vi tri cac the",
                "explanation": "Ghep tat ca cac cap",
            }
        ]
    ...
    def submit_answer(self, answer):
        ...
        if (
            i1 == i2
            or i1 in self._matched
            or i2 in self._matched
            or i1 < 0
            or i2 < 0
            or i1 >= len(cards)
            or i2 >= len(cards)
        ...
        if cards[i1]["symbol"] == cards[i2]["symbol"]:
            ...
            return AnswerResult(
                is_correct=True,
                correct_answer=cards[i1]["symbol"],
                explanation="Ghep dung!",
                next_state=ns,
                stars_earned=stars,
                hints_remaining=self._hints_remaining,
                retry_allowed=False,
        ...
        return AnswerResult(
            is_correct=False,
            correct_answer=None,
            explanation="Khong khop, thu lai",
            next_state=self._state,
            stars_earned=0,
            hints_remaining=self._hints_remaining,
            retry_allowed=True,
    ...
    def complete(self):
        ...
        return CompletionResult(
            total_stars=total,
            badges_unlocked=["Sieu tri nho"] if total > 0 else [],
            rewards_unlocked=[],
