...
class MathRaceGame(GameEngineBase):
    ...
    def _difficulty_band(self, level):
        if level <= 3: return 5, 10, ["+", "-"]
        elif level <= 5: return 5, 20, ["+", "-"]
        elif level <= 7: return 10, 100, ["+", "-"]
        elif level <= 9: return 50, 1000, ["+", "-", "x"]
        return -50, 1000, ["+", "-", "x", "/"]
    ...
    def _gen_problem(self, rnd, level):
        ...
        if op == "-":
            a, b = rnd.randint(lo, hi), rnd.randint(lo, hi)
            return str(max(a,b)) + " - " + str(min(a,b)) + " = ?", max(a,b) - min(a,b)
        if op == "x":
    ...
    def load_level(self, level):
        ...
        for _ in range(10):
            ...
            opts = [ans]
            for _ in range(3):
                d = ans + rnd.randint(1, max(2, abs(ans)//5 + 2)) * rnd.choice([-1, 1])
                if d not in opts:
                    opts.append(d)
                else:
                    opts.append(ans + rnd.randint(1, 5))
            rnd.shuffle(opts)
            items.append({"question": q, "options": opts, "correct_answer": ans, "hint": "Tinh toan ky nhe", "explanation": q + " = " + str(ans)})
